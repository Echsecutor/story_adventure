 ...
 */
export async function downloadAsIs(story: Story): Promise<void> {
 ...
 */
async function downloadMediaInSection(
  story: Story,
  currentIndex: number,
  sectionIds: string[],
  finalCallback: () => void
 ...
 */
export async function downloadGraphInOne(story: Story): Promise<void> {
...
export async function downloadGraphSplit(story: Story): Promise<void> {
  ...
  if (!manifest.files || Object.keys(manifest.files).length === 0) {
    throw new Error(
      'Viewer bundle manifest is empty. Please run: pnpm build:viewer-for-bundle'
    );
  }
...
export async function depthFirstSearch(
  ...
  if (currentSectionId === endAt) {
    ...
    if (passingThrough.length > 0) {
      for (const passing of passingThrough) {
        if (
          !linearizedHistory.includes(String(passing)) &&
  ...
  for (const choice of currentSection.next) {
    ...
    const foundPath = await depthFirstSearch(
      [...linearizedHistory, nextId],
      endAt,
      passingThrough,
      story
    );
 ...
 */
export function markdownFromSectionIdList(
  sectionIds: string[],
  story: Story
): string {
  let md = '';
 ...
 */
export async function downloadLinearStory(
  story: Story,
  startAt: string,
  endAt: string,
  passingThrough: string[]
): Promise<void> {
  const linearizedHistory = await depthFirstSearch(
    [startAt],
    endAt,
    passingThrough,
    story
  ...
  if (!linearizedHistory) {
    throw new Error(
      `Could not find a linear story which starts at ${startAt} and ends at ${endAt} while passing through all of ${passingThrough.join(', ')}`
    );
  }
